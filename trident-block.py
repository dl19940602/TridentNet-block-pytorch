...
import pdb
#from sharedweight_tridenet_block import trident_block#导入tridenet-block
...
class trident_block(nn.Module):
    expansion = 4
    def __init__(self, inplanes, planes, stride=1, downsample=None, padding=[0, 0, 0], dilate=[1, 2, 3]):
...
#  def __init__(self, block, layers, num_classes=1000):#layers数组，units个数
  def __init__(self, block, block1, layers, num_classes=1000):#layers数组，units个数
    self.inplanes = 64
...
def resnet_trident_101(pretrained=False):
  """Constructs a ResNet-101 model.
  Args:
    pretrained (bool): If True, returns a model pre-trained on ImageNet
  """
  model = ResNet(Bottleneck, trident_block, [3, 4, 23, 3])#论文采用15个trident-block
  if pretrained:
    model.load_state_dict(model_zoo.load_url(model_urls['resnet101']))
  return model
...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False):
    ...
    _fasterRCNN.__init__(self, classes, class_agnostic)#初始化

  def _init_modules(self):
#    resnet = resnet101()
     resnet = resnet_trident_101()#不加载预训练模型
    ...
    self.RCNN_base = nn.Sequential(resnet.conv1, resnet.bn1,resnet.relu,
      resnet.maxpool,resnet.layer1,resnet.layer2,resnet.layer3)#backbone(input->conv1->bn1->....->layer3)

    self.RCNN_top = nn.Sequential(resnet.layer4)#conv5

    self.RCNN_cls_score = nn.Linear(2048, self.n_classes)#fc层，类得分
    ...
    # Fix blocks
    for p in self.RCNN_base[0].parameters(): p.requires_grad=False#固定参数
    for p in self.RCNN_base[1].parameters(): p.requires_grad=False

    assert (0 <= cfg.RESNET.FIXED_BLOCKS < 4)#是否固定参数
  ...
  def _head_to_tail(self, pool5):#pool5为roi pooling
